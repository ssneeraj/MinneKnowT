...
public class QuizStatsActivity extends Activity {
	...
	private class QuizReaderTask extends AsyncTask<Void, Void, Boolean> {
		...
		protected void onPostExecute(Boolean result) {
			...
			TextView totalQuestionsAnsweredTV = (TextView) findViewById(R.id.quizStats_totalQuestionsAnsweredValue);
			totalQuestionsAnsweredTV.setText(buffer.toString());
			buffer.setLength(0);

			// -------------------------------------------
			// Set value for correct and incorrect text view

			// always remember to move the cursor so that it points
			// to the
			// first row
			cursor_correctIncorrect_table.moveToFirst();

			TextView correctTV = (TextView) findViewById(R.id.quizStats_total_correct_answers_value);
			...
			correctTV.setText(buffer.toString());

			buffer.setLength(0);

			TextView incorrectTV = (TextView) findViewById(R.id.quizStats_total_incorrect_answers_value);
			...
			incorrectTV.setText(buffer.toString());
			...
			// Compute the height for Green bar height and Red bar
			// height for correct and incorrect answers

			// Now compute what percentage of default bar height
			// will be green bar

			int correct_ans_percentage = (100 * total_correct_answers)
					/ total_questions_answered;

			int incorrect_ans_percentage = (100 * total_incorrect_answers)
					/ total_questions_answered;
			...
			quizStats_graph_greenBar.setImageBitmap(bitmapGreenBar);
			...
			quizStats_graph_redBar.setImageBitmap(bitmapRedBar);
